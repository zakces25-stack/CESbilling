...
(function (root) {
  ...
  function mount(host, opts) {
    ...
    host.innerHTML =
      '<div class="sq-card" id="sqInputCard">' +
        '<div class="sq-grid">' +
          ...
          '<div><label class="sq-lab">CES commission (p/kWh, in the rates)</label><input id="sqComm" class="sq-in" inputmode="decimal" placeholder="e.g. 1.00" title="What CES earns inside these rates. Recorded and shown; never added to a rate, because the supplier\'s prices already include it."></div>' +
        '</div>' +
      ...
      '<div class="sq-card sq-result" id="sqResultCard" style="display:none">' +
        '<div class="sq-top"><div><div class="sq-title" id="sqTitle">Electricity Price Analysis</div><div class="sq-sub" id="sqSubtitle"></div></div>' +
          ...
          '</div></div>' +
        '<div id="sqCommLine" class="sq-comm"></div>' +
        '<div id="sqWarn" style="padding:0 18px"></div>' +
        '<div style="overflow-x:auto;padding:0 18px 18px"><table class="sq-table" id="sqTable"></table></div>' +
        '<div class="sq-legend"><span class="sq-swatch"></span>A red figure is one the reader was not sure of, or whose unit it could not see. Click it to correct, or use Confirm all once checked. ' +
          ...
          'Gas is always one unit rate. "Commission included" is what CES earns inside the supplier\'s rates, in p/kWh: it is recorded here, never added to a rate.</div>' +
      '</div>';
    ...
    /** The commission recorded for a row: its own figure if the document gave one, else the desk's. */
    var rowCommission = function (r) { return r.com != null ? r.com : S.commission; };
    ...
    function render() {
      ...
      var hasCom = S.commission != null || shown.some(function (r) { return r.com != null || r.comIncluded != null; });
      ...
      var comCell = function (r) {
        if (r.current) return '<td style="color:#666">—</td>';
        var v = rowCommission(r);
        var txt = v != null ? Number(v).toFixed(2) : (r.comIncluded === true ? 'included' : (r.comIncluded === false ? 'excluded' : ''));
        return '<td class="edit' + (r.comLow ? ' low' : '') + '" data-id="' + r.id + '" data-key="com" data-dp="2" title="CES commission inside these rates, p/kWh. Recorded, never added to a rate. Click to edit this row\'s figure.">' + txt + '</td>';
      };
      var head = '<tr><th>Supplier</th><th>Standing Charge<br>p/day</th>' +
        ...
        (hasCom ? '<th>Commission<br>included p/kWh</th>' : '') +
      ...
      var body = rows.map(function (x) {
        ...
        var tip = [r.product ? 'Product as written: ' + r.product : '', r.source ? 'From: ' + r.source : '', r.supplierFromHint ? 'Supplier picked by you; the document did not name one' : ''].filter(Boolean).join('\n');
      ...
      // The commission line: what CES earns inside these rates, and what that is a year.
      var cl = $('sqCommLine');
      if (S.commission != null) {
        var kwh = totalKwh();
        cl.innerHTML = 'CES commission included in these quotes: <b>' + S.commission.toFixed(2) + ' p/kWh</b>' +
          (kwh ? ', about <b>' + money(kwh * S.commission / 100) + ' a year</b> on ' + kwh.toLocaleString('en-GB') + ' kWh' : '') +
          '. It is already inside the supplier\'s rates above and has not been added to them.';
      } else cl.innerHTML = '';

      var w = $('sqWarn');
      var lows = shown.filter(function (r) { return r.scLow || r.unitLow || r.dayLow || r.nightLow || r.capLow || r.termLow || r.comLow; }).length;
      w.innerHTML = (lows ? '<div class="sq-note red"><b>' + lows + ' row' + (lows === 1 ? ' has' : 's have') + ' a figure the reader was not sure of.</b> Check the red cells against the supplier\'s document, click to correct, then Confirm all.</div>' : '') +
    ...
    function edit(td, id, key, dp) {
      ...
      var current = key === 'com' ? rowCommission(r) : r[key];
  ...
  root.SupplierQuotes = { VERSION: VERSION, SUPPLIERS: SUPPLIERS, mount: mount, visibleSheets: visibleSheets, workbookText: workbookText, annualFor: annualFor };
